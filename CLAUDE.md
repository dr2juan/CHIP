...
3. **The Overview constellation** (rotating project map) is **vanilla JS at the
   ...
   **Do not hand-check this. Run `node tools/constellation-qa.js`** after installing
   ...
   - **Specimen-lineage edges (`l-src`) are a rule, not a one-off.** A project
     carrying `src:'<other-project-id>'` draws ONE solid orange line to that
     project, meaning exactly *"this project analyses specimens or data
     generated by that one"* (first use: `proj-gccpeh` ← `proj-air-heat`, whose
     screenings banked the buccal DNA the pilot analyses). Every other line on
     the map means "belongs to this domain" — do not add a third meaning
     without a legend entry. **The arrowhead is drawn as part of the path**,
     with its barbs sized in *pixels* and divided back through the per-axis
     scale (`SX`/`SY`, refreshed each render from the canvas box). That is the
     only reason an arrow works here: the svg is `viewBox="0 0 100 100"` with
     `preserveAspectRatio="none"`, so x and y scale independently and an
     `<marker>` would be visibly squashed and would change shape with the
     canvas aspect. Both ends are trimmed 9px so the head clears the node dot
     rather than hiding under it, and the stroke width matches the domain
     lines (1.1) — it read as heavy-handed at 1.7. The QA
